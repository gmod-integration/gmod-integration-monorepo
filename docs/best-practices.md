...
- run the affected app in dev (`turbo run dev --parallel` or a targeted script),
- verify no dependency graph cycle is introduced.

See also: [architecture.md](./architecture.md) for the "why" behind these rules, and
[migration-playbook.md](./migration-playbook.md) for moving code between `apps/*` and `packages/*`.
